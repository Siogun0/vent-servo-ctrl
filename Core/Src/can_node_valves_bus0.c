...
void can_node_valves_bus0_tx(volatile t_can_node_valves_bus0_output *out)
{
    ...
    //POWER_STATUS
    if((msg_cntr_power_status >= MSG_CYCLE_POWER_STATUS) || (out->tx_now.power_status))
    {
        msg_cntr_power_status -= out->tx_now.power_status ? msg_cntr_power_status : MSG_CYCLE_POWER_STATUS;
        out->tx_now.power_status = 0;
        
        msg = 0;
        
        can_node_power_status_cb(0x200 + __tx_id_shift__, msg, 8);
        
        //Sinals
        msg |= SetBits(0, 16, (out->POWER_STATUS.V_12V - 0.000000f) * (1 / 0.004000f));
        msg |= SetBitsSigned(16, 16, (out->POWER_STATUS.I_12V - 0.000000f) * (1 / 0.000100f));
        msg |= SetBitsSigned(32, 16, (out->POWER_STATUS.P_12V - 0.000000f) * (1 / 0.001000f));
        
        platform_can_xmit_mb(0, MBN_TX_POWER_STATUS + __mb_shift__, msg);
    }
}
